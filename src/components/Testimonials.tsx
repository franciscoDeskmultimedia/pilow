    ...
    <section id="testimonials" className="section bg-gradient-to-br from-pilow-slate to-pilow-slate-dark text-white overflow-hidden" aria-labelledby="testimonials-title">
      ...
      <div className="container relative z-10">
        <motion.div ref={ref} initial={{ opacity: 0, y: 30 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }} className="text-center mb-16">
          <span className="inline-block px-4 py-2 rounded-full bg-white/10 text-pilow-cyan text-sm font-medium mb-4">Testimonials</span>
          <h2 id="testimonials-title" className="section-title">What Our <span className="text-pilow-cyan">Clients</span> Say</h2>
          <p className="section-subtitle text-gray-300">Don&apos;t just take our word for it.</p>
        </motion.div>
        ...
        <div className="max-w-4xl mx-auto">
          <motion.div key={activeIndex} initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }} className="relative">
            ...
            <div className="glass bg-white/5 rounded-3xl p-8 md:p-12">
              <div className="flex gap-1 mb-6" role="img" aria-label={`${testimonials[activeIndex]?.rating || 5} stars`}>
                {[...Array(5)].map((_, i) => (
                  <svg key={i} className={`w-6 h-6 ${i < (testimonials[activeIndex]?.rating || 5) ? "text-yellow-400" : "text-gray-600"}`} fill="currentColor" viewBox="0 0 20 20">
                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                  </svg>
                ))}
              </div>
              <blockquote><p className="text-xl md:text-2xl leading-relaxed mb-8 font-light">&ldquo;{testimonials[activeIndex]?.content}&rdquo;</p></blockquote>
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-full bg-gradient-to-br from-pilow-ocean to-pilow-cyan flex items-center justify-center text-white font-bold text-xl">{testimonials[activeIndex]?.author?.charAt(0)}</div>
                <cite className="not-italic">
                  <div className="font-semibold text-lg">{testimonials[activeIndex]?.author}</div>
                  <div className="text-pilow-cyan">{testimonials[activeIndex]?.role}, {testimonials[activeIndex]?.company}</div>
                </cite>
              </div>
            </div>
          </motion.div>
          ...
          <div className="flex items-center justify-between mt-8">
            ...
            <div className="flex gap-3">
              <button onClick={() => setActiveIndex((prev) => (prev - 1 + testimonials.length) % testimonials.length)} className="w-12 h-12 rounded-full border border-white/30 flex items-center justify-center hover:bg-white/10" aria-label="Previous">
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
              </button>
              <button onClick={() => setActiveIndex((prev) => (prev + 1) % testimonials.length)} className="w-12 h-12 rounded-full border border-white/30 flex items-center justify-center hover:bg-white/10" aria-label="Next">
